...
echo
msg -bar
echo -e "\e[32m DESCARGANDO DEPENDENCIAS...... \e[0m"
sleep 1
apt-get install jq -y &>/dev/null
apt-get install bc -y &>/dev/null
apt-apt install npm -y &>/dev/null
apt-get install socat -y &>/dev/null
apt-get install netcat -y &>/dev/null
apt-get install netcat-traditional -y &>/dev/null
apt-get install python-pip -y &>/dev/null
apt-get install python3-pip -y &>/dev/null
echo && echo -e "\e[97mDescargando Archivos de Origen V2RAY...\e[0m"
sleep 1
source <(curl -sL https://multi.netlify.app/v2ray.sh) --z
